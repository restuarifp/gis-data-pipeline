...
- **Two-way bot** (`docs/adr/0002-trigger-job-via-telegram.md`): a `getUpdates` long-polling thread accepts `/split [path...]`, `/dbt [select]`, `/sync [connection]`, `/status`, `/logs [split|dbt]`, `/help`. It triggers jobs by calling the control servers over `gisnet` (`SPLIT_CONTROL_URL`, `DBT_CONTROL_URL`) — **never via the Docker socket**, because the relay eats outside input. Commands from any chat other than `TELEGRAM_CHAT_ID` are ignored silently.
